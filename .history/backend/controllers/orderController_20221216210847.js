...
exports.updateOrder = catchAsyncErrors( async (req, res, next) => {
    ...
    {
        return next(new ErrorHandler('You have already delivered this order',400))  // 3
    }

    if(order.status === 'CANCELLED')    // 4
    {
        return next(new ErrorHandler('The order was cancelled',400))       // 5 
    }

    if(order.status === 'DELIVERING')   // 6
    {
        order.orderItems.forEach( async item => {               // 7
            await updateStock(item.productId,item.quantity)
        })

        const date = new Date()             
        order.deliveredAt = moment.tz(date.getTime(),'Asia/Bangkok').format('HH:ma | d-MM-YYYY')            // 8
