...
exports.deleteItemCart = catchAsyncError( async (req, res, next) => {
    ...
    const listItem = getCart.products       // 3
    const findItemCart = listItem.findIndex( item => item.productId == req.params.id)  // 4

    if(!getCart){                           // 5
        return next(new ErrorHandler('Cart not found', 404))    // 6
    }
...
exports.decreaseQuantity = catchAsyncError( async (req, res, next) => { 
    ...
    // get user current
    const  userId  = req.user[0]._id        // 2

     // cart is already
    const cartOld = await Cart.findOne({        // 3
        user: userId
    })
