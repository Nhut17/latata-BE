...
const Introduce = ({data}) => {
    ...
    const currentProduct = listCartUser?.products.filter(val => val.productId == id)
    
    const [isStock,setIsStock] = useState(data.stock - currentProduct[0]?.quantity)
    console.log('quantity product ' + typeof isStock)
    ...
    return (
        <div className='product-introduce flex  j-between bd-bottom'>
            ...
            <div className="product-detail-content">
                ...
                {
                    isStock && 
                    isStock <= 0 &&   <button className='btn-buy-now' 
                    onClick={handleBuy}
                    disabled={true}
                    style={{
                        cursor: 'not-allowed',
