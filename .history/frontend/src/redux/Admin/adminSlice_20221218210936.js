...
// update product
export const updateProduct = createAsyncThunk('admin/product/create',
                 async(data,thunkAPI) => {
                        try{
                            const token = localStorage.getItem('token')
                            const config = {
                            ...
                            const { name,price,promotion,stock,productId } = data
                           
                            const res = await api.put('/api/v1/admin/product/new',data,config)
